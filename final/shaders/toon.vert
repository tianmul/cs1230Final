    ...
    if (useLighting) {
        ...
        for (int i = 0; i < MAX_LIGHTS; i++) {
            ...
            if (diffuseIntensity > 0.95)
                    color += max(vec3(0), lightColors[i] * diffuse_color * 0.95);
            else if (diffuseIntensity > 0.5)
                    color += max(vec3(0), lightColors[i] * diffuse_color * 0.5);
            else if (diffuseIntensity > 0.25)
                    color += max(vec3(0), lightColors[i] * diffuse_color * 0.25);
            else
                    color += max(vec3(0), lightColors[i] * diffuse_color  * 0.1);
