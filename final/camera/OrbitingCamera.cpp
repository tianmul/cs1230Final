...
void OrbitingCamera::mouseScrolled(float deltaX, float deltaZ) {
    ...
    glm::vec4 temp= glm::normalize(glm::transpose(m_viewMatrix) * glm::vec4(0,0,1,0));
    glm::vec4 temp2= glm::normalize(glm::transpose(m_viewMatrix) * glm::vec4(1,0,0,0));

    eyepos -= deltaZ*temp.xyz();
    eyepos += deltaX*temp2.xyz();
    updateViewMatrix();
}
...
void OrbitingCamera::updateProjectionMatrix() {
    ...
    float farPlane = glm::max(50.0f, 0.1f+ 100.f * FLT_EPSILON);
    float h = farPlane * glm::tan(glm::radians(50.0f * 0.5f));

    float w = m_aspectRatio * h;

    m_scaleMatrix = glm::scale(glm::vec3(1.f / w, 1.f / h, 1.f / farPlane));
    m_projectionMatrix = glm::perspective(
            glm::radians(50.0f), m_aspectRatio, 0.1f, farPlane) * 0.02f;
}

void OrbitingCamera::updateViewMatrix() {
    m_viewMatrix =
            glm::rotate(glm::radians(m_angleY), glm::vec3(0.f, 1.f, 0.f)) *
            glm::rotate(glm::radians(m_angleX), glm::vec3(1.f, 0.f, 0.f)) *glm::transpose(glm::mat4x4(
                                                                                               1,0,0,-eyepos.x,
                                                                                               0,1,0,-eyepos.y,
                                                                                               0,0,1,-eyepos.z,
                                                                                               0,0,0,1));
